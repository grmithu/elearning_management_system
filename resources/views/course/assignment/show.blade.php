    ...
    @if(auth()->id() == $course->instructor->id)
        <div class="card p-3">
            <div class="card-header">
                <h5 class="card-title pb-0 fs-4">Update {{ $assignment->name }}</h5>
                <p class="small">{{ $assignment->name }} Information</p>
            </div>
            <div class="card-body mt-4">
                <form class="row g-3 needs-validation" method="POST" action="{{ route('assignment.update', [$course->id, $assignment->id]) }}" novalidate enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="col-12 col-md-6">
                        <label for="title" class="form-label">Title <span class="text-danger small">*</span></label>
                        <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title') ?? $assignment->name }}" required>
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <input type="number" step="any" class="form-control @error('total_marks') is-invalid @enderror" id="total_marks" name="total_marks" value="{{ old('total_marks') ?? $assignment->total_marks }}" required min="1">
...
{{--                        <input type="number" class="form-control @error('pass_marks') is-invalid @enderror" id="pass_marks" name="pass_marks" value="{{ old('pass_marks') ?? $assignment->pass_marks }}" required min="1">--}}
...
{{--                    </div>--}}
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <input type="datetime-local" class="form-control @error('start_time') is-invalid @enderror" id="start_time" name="start_time" value="{{ old('start_time') ?? $assignment->valid_from }}" required>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="row">
                            <div class="col-12">
                                ...
                                <input type="datetime-local" class="form-control @error('end_time') is-invalid @enderror" id="end_time" name="end_time" value="{{ old('end_time') ?? $assignment->valid_upto }}" required>
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="6">{{ old('description') ?? $assignment->description }}</textarea>
        ...
        <div class="card p-3">
            <div class="card-header">
                <h5 class="card-title pb-0 fs-4">
                    Submissions in {{ $assignment->name }}
                    @if(count($assignment->attempts))
                        <small class="text-muted">({{ count($assignment->attempts) }})</small>
                    @endif
                </h5>
            </div>
            <div class="card-body mt-4">
                            ...
                            <div class="col-12 col-sm-6 col-md-4 col-lg-3 my-3">
                                <div class="shadow-sm rounded-3 overflow-hidden">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <a href="{{ route('assignment.show.submission', [$course->id, $assignment->id, $attempt->id]) }}" class="btn btn-sm btn-primary mt-3 d-block">See Details</a>
                                    </div>
                                </div>
                            </div>
                        @endforeach
    ...
    @else
        <div class="card p-3">
            <div class="card-header">
                <h5 class="card-title py-0 text-capitalize">{{ $assignment->name }}</h5>
            </div>
            <div class="card-body mt-4">
                <pre style="text-align: justify">{!! $assignment->description !!}</pre>
            </div>
        </div>
        <div class="card p-3">
            <div class="card-header d-flex justify-content-between">
                <div>
                    <h5 class="card-title pb-0 fs-4">Submit your "{{ $assignment->name }}" Assignment</h5>
                </div>
                <div class="text-end">
                    <small class="text-success">Total Marks : {{ $assignment->total_marks }}</small>
{{--                    <small class="text-primary">Pass Marks : {{ $assignment->pass_marks }}</small>--}}
                    @php
                        $hours = floor($assignment->duration / 60);
                        $minutes = $assignment->duration % 60;

                        $student_attempt = $assignment->attempts->where('participant_id', auth()->id())->first();
                    @endphp
...
{{--                            <small class="d-block">Status : <span class="{{ $student_attempt->obtained_marks < $assignment->pass_marks ? 'text-danger' : 'text-success' }}">{{ $student_attempt->obtained_marks < $assignment->pass_marks ? 'Failed' : 'Passed' }}</span></small>--}}
{{--                        @endif--}}
                    @endif
                </div>
            </div>
            ...
            <div class="card-body mt-4">
                @if( \Carbon\Carbon::now()->lte($assignment->valid_upto) && !$student_attempt)
                    <form class="row g-3 needs-validation" method="POST" action="{{ route('assignment.answer.store', [$course->id, $assignment->id]) }}" novalidate enctype="multipart/form-data">
                        @csrf
                        <div class="col-12 col-md-8 offset-md-2">
                            <label for="document" class="form-label">Assignment File <span class="text-danger small">*</span></label>
                        ...
                        <div class="col-12 col-md-8 offset-md-2">
                            <label for="answer_description" class="form-label">Assignment Description</label>
                ...
                @elseif($student_attempt)
                    <div>
                        <div class="text-center">
                            ...
                            <a href="{{ route('assignment.download.pdf', [$course->id, $student_attempt->pdf]) }}" class="btn btn-primary btn-sm shadow">Download File</a>
                        </div>
