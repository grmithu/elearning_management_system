...
@section('main')
    <div class="card p-3">
        ...
        <div class="card-body mt-4">
            <span class="text-muted d-block">Total Marks : {{ $assignment->total_marks }}</span>

            <form action="{{ route('assignment.store.submission.mark', [$course->id, $assignment->id, $submission->id]) }}" method="Post" class="form-inline needs-validation" novalidate>
                @csrf
                <div class="form-group d-flex flex-column flex-sm-row align-items-center">
                    ...
                    <div class="px-3 my-2">
                        <input type="number" step="any" class="form-control @error('obtained_mark') is-invalid @enderror" id="obtained_mark" name="obtained_mark" max="{{ $assignment->total_marks }}" value="{{ $submission->obtained_marks }}" required>
                        @error('obtained_mark')
            ...
            <div class="text-center">
                ...
                <a href="{{ route('assignment.download.pdf', [$course->id, $submission->pdf]) }}" class="btn btn-primary btn-sm shadow">Download File</a>
            </div>
