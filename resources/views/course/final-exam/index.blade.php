    ...
    @if (count($course->enrollees))
        <div class="card p-3">
            <div class="card-header d-flex justify-content-between">
                <h5 class="card-title pb-0 fs-4">Final Exam Mark</h5>
            </div>
                        ...
                        <tr class="{{ auth()->id() == $enrollee->id ? 'bg-success-light' : '' }}">
                            ...
                            <td class="align-middle fw-bold">
                                @if(auth()->id() == $course->instructor?->id)
                                    <form class="g-3 needs-validation" action="{{ route('final-exam.store.mark-release', $course->id) }}" method="POST" novalidate>
                                        @csrf
                                        <div class="d-inline-block">
                                            <input type="hidden" name="student_id" value="{{ $enrollee->id }}" required>
                                            <input type="number" step="any" name="obtained_mark" class="form-control @error('obtained_mark') is-invalid @enderror" value="{{ old('obtained_mark') ?? $course->finalExamMarks->where('participant_id', $enrollee->id)->first()?->obtained_marks }}" required>
                                        </div>
                                        <div class="d-inline-block">
                                ...
                                @else
                                    {{ $course->finalExamMarks->where('participant_id', $enrollee->id)->first()?->obtained_marks ?? 'N/A' }}
                                @endif
                            </td>
                        </tr>
                    @endforeach
